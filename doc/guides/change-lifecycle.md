...
- **Mode-aware, never-ticket handoff:** `@doc-syncer` reads `delivery_mode` from `chg-<workItemRef>-pm-notes.yaml` (absent ⇒ `interactive`). **Resolution is mode-aware** — in `autonomous` mode a detected gap for a modified feature area with **no** spec is **resolved in-change**: the missing `doc/spec/features/feature-<slug>.md` is authored (first-spec-only; an existing spec is reconciled, never re-authored; authoritative behavior lives in `.opencode/agent/doc-syncer.md`). In `interactive`/absent mode the handoff is unchanged (report-only): `@pm` checks open issues for an existing tracker and **references** it (rather than proposing a duplicate — de-noising), **proposes** a follow-up to the human, and **only the human** approves ticket creation. In **both** modes `@doc-syncer` never creates a tracker ticket and no agent creates a ticket in any mode. **"Advisory" means non-blocking at this phase / the human decides ticket creation** (interactive) — it must **not** be read as "can be silently skipped" (in autonomous mode the gap is resolved in-change, not dropped). Coverage does not block the change from proceeding to `review_fix`.
