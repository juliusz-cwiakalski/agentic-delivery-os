...
- **G-3**: Preserve byte-identical delivery behavior for users who do not opt into quota checking (no `ZAI_API_KEY`): same gate, same sleep target, same exit status, no network call, no `jq`, no new runtime dependency.
...
- **G-6**: Handle provider-secret material safely (never log the full key or the raw response body in production).
...
| Behavior delta for non-opt-in users (no `ZAI_API_KEY`) | None — peak-only path is byte-identical (same gate, sleep target, exit 0; no network, no `jq`) |
...
**F-5 — Safe secret handling.** The hook never writes the full `ZAI_API_KEY` to stderr or stdout; only a short prefix/suffix may appear in a diagnostic. In production mode the hook never logs the raw response body (the body may contain account-identifying detail and is not needed for operator diagnostics once a `[WARN]` reason is emitted). A redacted reason category (e.g. "HTTP 401", "malformed JSON") is sufficient.
...
- Secret-handling guarantees: never log the full key; never log the raw response body in production.
...
- **Fully backward compatible.** Users who do not set `ZAI_API_KEY` get byte-identical delivery behavior: the quota condition returns `0` silently, performs no network call, and spawns no `jq`; the peak path is unchanged pure-bash.
...
| NFR-7 | Secret redaction | stderr/stdout contain 0 occurrences of the full `ZAI_API_KEY` (prefix/suffix only) and, in production mode, 0 occurrences of the raw response body |
...
| RSK-4 | Secret leakage — full `ZAI_API_KEY` or raw response body reaches logs | H | M | Never log the full key (prefix/suffix only); never log raw body in production; AC + tests assert redaction (F-5, NFR-7, AC-F5-1) | Low |
...
| RSK-6 | Refactor changes peak behavior for existing users | H | L | Peak logic moves into the condition contract with behavior-preserving AC (AC-F2-1); peak path tested for byte-identical observable behavior; opt-out users hit no `jq`/network (NFR-3) | Low |
...
| DEC-10 | Security: never log the full `ZAI_API_KEY` (prefix/suffix only); never log raw response body in production | Secret hygiene; `set-evn.sh` (real key) is never staged/committed | 2026-08-04 |
...
| AC-F5-1 | **Given** any code path that handles `ZAI_API_KEY` or the response body, **when** the hook emits stderr/stdout, **then** it never contains the full `ZAI_API_KEY` (prefix/suffix only) and never contains the raw response body in production mode. `[F]` | F-5, NFR-7 |
...
- The raw response body (which may contain account-identifying detail) is never logged in production (F-5, NFR-7).
