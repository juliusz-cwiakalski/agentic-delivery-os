...
- **(C) Surgical guide amendments** to `doc/guides/project-inception.md` (4 edits, preserving `ados_distribution: redistributable`): (a) the artifact-catalog row for tribal-knowledge references `tribal-knowledge-template.md` instead of an em-dash; (b) the "Tribal-knowledge graduation (Phase 0→1)" label is corrected to Phase 2 (contradiction fix vs the shipped agent + spec); (c) a brief trust/safety note in the legacy section (untrusted input + secrets refusal) so human authority matches agent authority; (d) the legacy-flow-differences table row (~line 654) conditions extraction on "PR/MR history" — correct to "repo docs + git history (`git log`)" since PR/MR history is GH-33's parked surface, not GH-72's scope (contradiction fix; RT1-M2).
...
| DEC-4 | Guide amendments are surgical and meet the GH-71 DEC-5 bar (concrete AND blocking): the em-dash template column, the "Phase 0→1" label, the missing trust/safety note, and the "PR/MR history" extraction-source label (~line 654, which misattributes the source to GH-33's parked surface) are all concrete contradictions/factual errors vs the shipped agent + spec. | Preserves the just-shipped guide while removing genuine contradictions (NFR-7). RT1-M2 surfaced the 4th amendment. | 2026-06-27 |
...
|----|----------------------------------|--------|
| AC1 | **Given** `project.flow: legacy` with no pre-existing hand-authored `tribal-knowledge.md`, **when** Phase 0 runs, **then** it produces `doc/inception/analysis/tribal-knowledge.md` from in-repo docs + git history using file reads + `git log` only (no PR-thread tooling), and it is reviewed at gate 0. (If a hand-authored file already exists, PRODUCE preserves it per OQ-2.) **Verification: structural (Phase-0 legacy produce step present; `new` branch has none) + manual (agent behavior).** | F-1, NFR-1, NFR-3 |
