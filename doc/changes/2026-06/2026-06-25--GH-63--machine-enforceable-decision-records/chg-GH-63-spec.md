...
**F-3 — `validate-decision-record`.** A PATH-able CLI in `tools/` (no `.sh` extension) following `doc/guides/tools-convention.md` (with DEC-14 exception) and `.ai/rules/bash.md`. **Default mode = front-matter validation only** (DEC-16); planning-summary validation requires explicit `--summary`/a summary-artifact input. It enforces: (a) front-matter structural validity (all DM-1 rules); (b) rigor-aware required fields — `decision_date` non-null when `status=Accepted`; `governance.decider` present when `status=Accepted` AND `classification.rigor ∈ {R2, R3}`; `governance.reviewers` non-empty when `status=Accepted` AND `classification.rigor = R3` (acceptance-gated, mirroring the decider rule — ADR-0001 is Proposed R3 with empty reviewers and legitimately passes); (c) lifecycle validity — only permitted transitions (DM-5) and `supersedes`/`superseded_by` consistency; (d) planning-summary structure (generic + legacy alias) including `hard_requirements ∩ decision_drivers = ∅` and best-effort non-negotiable-constraint violation detection against the chosen option; (e) a documented **heuristic** that an Accepted record's body contains a non-empty `## Verification Criteria`. Hard-rule failures emit an **actionable** message naming the record, the offending field, and the violated rule, and return a non-zero exit code (NFR-8); heuristic conditions emit non-blocking `[WARN]`/`[HEURISTIC]` messages with exit 0 (DEC-10/DEC-13). Runs on **stdlib only** — bash + python3 and/or jq, no `jsonschema` dependency (SD-4). Flags: `--help`, `--version`, `--dry-run`, `--summary` (validate a planning-summary artifact), `--lint` (non-destructive migration-lint mode), `--coverage` (emit schema-vs-validator coverage summary). Semantic exit codes; testable main guard. Validated by tests at `tools/.tests/test-validate-decision-record.sh` with positive and negative fixtures.

**F-4 — `generate-decision-index`.** A PATH-able CLI in `tools/` (no `.sh` extension) that reads front matter across `doc/decisions/*.md` and emits a deterministic Markdown index plus a health view. The **committed** `doc/decisions/00-index.md` (the drift-checked artifact) contains ONLY byte-stable content: the table (ID, Type, Title, Status, Date, Owners) + the **time-independent** health dimensions — missing deciders (Accepted R2/R3 without `governance.decider`), missing metrics, and the future-field-aware waiver dimension (empty today). The **time-dependent** dimension — overdue reviews (`review_date` in the past) — is emitted to **stdout / `--dry-run` only**, NEVER written to the committed `00-index.md`, so calendar drift cannot trip the CI gate (DEC-15). Output is **byte-stable** for the same input set (sorted, stable formatting, deterministic date handling). Supports `--help`, `--version`, `--dry-run`, `--summary` (full health report incl. overdue to stdout), semantic exit codes, and a testable main guard; tested at `tools/.tests/test-generate-decision-index.sh`.
...
| `links.metrics` | array<string> | optional | Metric IDs/docs (the index's "missing metrics" dimension flags Accepted records whose `links.metrics` is absent/empty — distinct from the validator's body `## Verification Criteria` heuristic, AC-GH63-6) |
...
>
> Note (m6): `classification.archetype` lists a forward-looking superset (sourced from the research taxonomy) beyond the few values illustrated in the template/ADR-0001. This is a **permissive** enum (no record is rejected for using an unlisted value beyond a warning) and does **not** pre-empt a future separate taxonomy-schema effort; it is flagged as a forward-looking extension, consistent with SD-1's "superseded-by-landed" rule applying to the *structure*, not to this explicitly-additive value set.
>
> Note (m9): `decision_area` has no `technical` value even though a TDR record type exists — `decision_area` classifies the *business domain* (architecture/product/business/operations/mixed), which is orthogonal to the record *type* prefix. TDR authors map to the closest area (commonly `architecture` or `mixed`).
...
|----|---------|-------------|
| DM-3 | Index output model | **Committed (byte-stable, drift-checked)** `00-index.md` = table (ID, Type, Title, Status, Date, Owners) + time-independent Health (missing deciders; missing metrics via `links.metrics`; future-field-aware waivers). **Advisory (stdout/`--dry-run` only, NOT committed)** = the time-dependent "overdue reviews" dimension. Split per DEC-15 so calendar time cannot cause CI drift. |
| DM-4 | Default-rigor rule | When `classification` is absent → `classification.rigor` is treated as **R2** for validation purposes (DM-1, NFR-2); existing records never rejected for being un-classified. **Caveat:** defaulting to R2 imposes R2 obligations on such records — an un-classified *Accepted* record must still satisfy R2 requirements (e.g., `governance.decider`). The sole live record (ADR-0001) is `Proposed`, so NFR-2's "100% of existing records valid" holds; future un-classified Accepted records must satisfy R2 rules. |
| DM-5 | Permitted lifecycle transitions | `Proposed → Under Review`; `Under Review → Accepted`; `Under Review → Proposed` (regress allowed); `Accepted → Deprecated`; `Accepted → Superseded` (via `superseded_by`); `Deprecated → Superseded` (rare). **`Accepted` is terminal** (F-3 immutability) — `Accepted → Proposed`/`Accepted → Under Review` are **invalid** (require a new superseding record). `Deprecated`/`Superseded` are also terminal. The validator rejects any record whose current `status` could not have been reached, but does NOT require a full transition history (single-snapshot validation). |
...
|----|-------------|-----------|
| NFR-1 | Git-native, no proprietary runtime, no network calls, no secrets (ADR-0001 C-4) | 0 required network calls at runtime (the two tools omit the automatic version-check per DEC-14); 0 secrets; 0 new runtime services; 0 proprietary-binary artifacts |
...
| NFR-4 | Tools follow `doc/guides/tools-convention.md` + `.ai/rules/bash.md` (with the documented exception DEC-14: no automatic network version-check, since these are repo-internal delivery tools, not standalone-installable utilities) | No `.sh` extension; `--help`/`--version`/`--dry-run`/`--summary`; semantic exit codes; testable main guard; embedded test framework; tests under `tools/.tests/` |
...
| DEC-14 | The two new tools (`validate-decision-record`, `generate-decision-index`) **omit the automatic network version-check** mandated by `tools-convention.md` §"Automatic Version Check" | Those tools are repo-internal delivery tooling (used in CI + locally within the repo), NOT standalone-installable system utilities (the convention's stated target — `zclaude`/`text-to-image`). The version check fetches `raw.githubusercontent.com` on every invocation, which would violate NFR-1 ("0 network calls") and is meaningless for a tool that lives in the repo it checks against (version = git HEAD). A documented exception; tool docs note it. Resolves the M3 NFR-1↔tools-convention catch-22. | 2026-06-25 |
| DEC-15 | The committed, drift-checked `doc/decisions/00-index.md` contains **only time-independent** content (table + missing-deciders/missing-metrics/future-waivers); the **time-dependent** "overdue reviews" dimension is **stdout/`--dry-run` only** | NFR-5 (byte-stability) and AC-GH63-12 (index-drift CI failure) both apply to the committed artifact; a time-dependent dimension in it would cause false-positive CI failures as calendar time advances (red-team M1). Splitting advisory-vs-committed preserves both determinism and the overdue-review signal. | 2026-06-25 |
| DEC-16 | Default mode of `validate-decision-record` = **front-matter only**; planning-summary validation requires an explicit `--summary`/summary-artifact input | A void default scope (no live planning-summary blocks persist under `doc/`) would make default mode ambiguous; explicit `--summary` makes AC-GH63-13 ("CI does not run planning-summary validator over live docs") behaviorally unambiguous. | 2026-06-25 |
...
| AC-GH63-5 | **Given** each IN-SCOPE §28.3 negative case that HARD-FAILS (invalid decision_type; invalid status; impossible lifecycle transition incl. supersedes/superseded_by inconsistency; missing owners; missing decider for Accepted R2/R3; missing decision_date for Accepted; Accepted R3 without governance.reviewers (acceptance-gated); same factor as both constraint and driver), **when** `tools/validate-decision-record` runs against a fixture exhibiting that case, **then** it fails (non-zero exit) with an actionable error naming the record, field, and violated rule. (The two best-effort cases — non-negotiable-violation and verification-criteria presence — are WARNINGS covered by AC-GH63-6, not hard failures.) | F-3, NFR-8, SD-2, Appendix A, DEC-12, DEC-13 |
| AC-GH63-6 | **Given** an Accepted decision record exhibiting a best-effort heuristic condition (body lacks a non-empty `## Verification Criteria`; OR the planning summary shows the chosen option violating a `negotiable: no` constraint), **when** the validator runs, **then** it **warns** (non-blocking, exit 0) via a clearly-labeled `[HEURISTIC]`/`[WARN]` message (per DEC-10/DEC-13, heuristics never fail the build — a failing check would be a structural guarantee these checks cannot provide without body-content/snapshot machinery owned by GH-64). | F-3, DEC-10, DEC-13, RSK-6 |
...
| AC-GH63-10 | **Given** `tools/generate-decision-index`, **when** it scans the corpus, **then** the **committed** `00-index.md` health view flags missing deciders (Accepted R2/R3 without `governance.decider`), missing metrics (`links.metrics` — Accepted records whose `links.metrics` is absent/empty; this is distinct from the validator's body `## Verification Criteria` heuristic at AC-GH63-6), and the future-field-aware waiver dimension (empty today) — all time-independent — AND the time-dependent "overdue reviews" dimension is emitted to **stdout/`--dry-run` only**, never written to the committed artifact (DEC-15). | F-4, DM-3, DEC-11, DEC-15 |
...
| AC-GH63-13 | **Given** the CI gate AND the default invocation mode, **when** `validate-decision-record <file>` runs (no `--summary`), **then** it validates **front matter only** (default mode per DEC-16); CI runs the front-matter validator over `doc/decisions/*.md` + schemas and the index drift check, and does **not** run the planning-summary validator over live docs (its correctness is proven by synthetic fixtures under `tools/.tests/`). Planning-summary validation requires explicit `--summary`/a summary-artifact input. | SD-3, DEC-3, DEC-16, F-6 |
...
| 9 | non-negotiable-constraint violation in chosen option | IN-SCOPE (heuristic/warning) | F-3 / AC-GH63-6 / DEC-13 |
...
> Totals: **8 IN-SCOPE hard-fail** + **2 IN-SCOPE heuristic/warning** (verification-criteria presence; non-negotiable-violation) + **4 DEFERRED** = 14. This satisfies the ticket AC "fails each negative case from §28.3" for every case expressible against landed artifacts: 8 hard-fail (AC-GH63-5) + 2 warning (AC-GH63-6); the four deferred cases require sibling machinery explicitly listed as out-of-scope/non-goals (formalized in PDR-0001).
