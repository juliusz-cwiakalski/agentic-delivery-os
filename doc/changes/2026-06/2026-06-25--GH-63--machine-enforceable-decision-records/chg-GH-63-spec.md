...
| AC-GH63-10 | **Given** `tools/generate-decision-index`, **when** it scans the corpus, **then** the **committed** `00-index.md` health view flags missing deciders (Accepted R2/R3 without `governance.decider`), missing metrics, and the future-field-aware waiver dimension (empty today) — all time-independent — AND the time-dependent "overdue reviews" dimension is emitted to **stdout/`--dry-run` only**, never written to the committed artifact (DEC-15). | F-4, DM-3, DEC-11, DEC-15 |
