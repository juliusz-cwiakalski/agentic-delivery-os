...
- [x] **8.1** Audit **all 5 existing Mermaid fences** (not ~3) against the shared convention set: `doc/guides/change-lifecycle.md` (**1 fence**, ~line 50) + `doc/guides/project-inception.md` (**4 fences**, ~lines 54, 164, 330, 667). Enumerate each fence explicitly when reviewing. Note any divergence in direction, color meaning, arrow meaning, label style, or a missing Legend / non-color cue (a11y). (audited all 5: change-lifecycle uses flowchart TD + subgraphs + `<br/>` + dashed feedback + Legend, no fills; project-inception L54 convergence / L166 master-flow / L332 Phase-0 intake / L669 shared back-half — all use the identical palette #4CAF50/#2196F3/#FF9800/#F44336/#9C27B0 with non-color text cues already present on red/green nodes; gap = no explicit Legend block on the 4 project-inception fences)
- [x] **8.2** Audit all newly added diagrams (Phases 1–5) for the same. (new diagrams — master map, README compact, meeting before/during/after, decision D0–D14+R0–R3, onboarding setup flow — all use the shared palette, quoted special-char labels, `<br/>`, Legend + non-color cues)
- [x] **8.3** Reconcile divergences with minimal edits: align colors to the shared palette meaning (green=entry/success, blue=process, orange=gate, red=remediation, purple=cross-cutting/optional); ensure dashed arrows mean feedback/optional only; ensure every diagram has a Legend or caption. Prefer adjusting the new diagrams to match where the existing ones already encode a clear convention; adjust existing ones only if a clear inconsistency exists. (palette already identical across new + existing — no color edits needed; added a concise accurate Legend caption after each of the 4 project-inception fences so every diagram carries a Legend — markdown after the closing fence, zero render-risk, no diagram internals touched; no redesign)
- [x] **8.4** Confirm no diagram uses forbidden syntax (click callbacks, `<a>` in labels, `%%{init}%%`, exotic shapes). (verified 0 forbidden directives INSIDE any mermaid block across all 7 files; whole-file grep hits are false positives — README hero `<a>` image tag + "one-click" prose + meeting "one click away" prose, none inside mermaid blocks)
- [x] **8.5** Confirm node naming is consistent: use the canonical process names ("Project Inception", "Onboarding", "Change Lifecycle", "Decision-Making", "Meetings", "Documentation Reconciliation") everywhere they appear across diagrams. (canonical process names used consistently in master map + README: Project Inception / Project Onboarding / Change Delivery / Meeting Management / Decision Making / Documentation Reconciliation — case-sensitive grep confirms presence; existing phase/track diagrams reference phases not process names, so no naming conflict)
...
- [x] **10.1** Run the doc-distribution drift guard: `bash scripts/.tests/test-doc-distribution.sh`. It must exit 0 (enforces `ados_distribution` marker presence, valid enum, redistributable docs are installed, no internal docs installed, derived set == sandbox install set). If it fails, fix the named condition and re-run until green. (PASS: `[OK] no drift — 76 in-scope docs; install set matches ados_distribution markers`; exit 0)
- [x] **10.2** Run the other repo test scripts to confirm no regressions: `bash scripts/.tests/test-add-header-location.sh`, `bash scripts/.tests/test-inception-doc-consistency.sh`, and any other `scripts/.tests/test-*.sh` / `tools/.tests/test-*.sh` that are quick and relevant (the text-to-image/zclaude suites are out of scope for a docs change but should remain green if run). Note results in the Execution Log. (test-add-header-location.sh 19/19 PASS; test-inception-doc-consistency.sh PASS exit 0; `git diff --check` exit 0 — no regressions; install/zclaude/build-plugin suites are out of scope for a docs change and were not modified)
- [x] **10.3** Spec reconciliation: confirm there is no `doc/spec/features/**` feature spec to update — process maps are guide-level documentation, not behavior specs, so `doc/spec/**` is unaffected. If the `@doc-syncer` (lifecycle phase 7) flags any coverage gap, record it as advisory (coverage does not block a docs change). State explicitly: "no `doc/spec/**` reconciliation required for this change." (confirmed: zero `doc/spec/**` changes in the diff; process maps are guide-level docs, not behavior specs — no `doc/spec/**` reconciliation required for this change)
- [x] **10.4** Version bump: none (docs-only; no versioned artifact per repo conventions). State explicitly: "no version bump — documentation-only change." (no version bump — documentation-only change; `version_impact: none` per the spec)
- [x] **10.5** Final DoD self-check: walk all 13 canonical acceptance criteria (`AC-F1-1`…`AC-F5-1`, `AC-NFR4-1`, `AC-NFR5-1` — Definition of Done below); confirm every checkbox in Phases 1–9 is checked; confirm the branch is `docs/GH-85/ados-process-map-diagrams`. (all 13 ACs PASSED with structural evidence — see DoD cross-walk notes; all Phase 1–9 checkboxes checked; branch confirmed `docs/GH-85/ados-process-map-diagrams`)
- [x] **10.6** Stage only the documentation files touched by this change (the new guide + the modified guides + README + doc/00-index.md). Do not stage unrelated files. (all deliverables committed per-phase; working tree contains only PM-owned `chg-GH-85-pm-notes.yaml` modification + `readiness-review/` untracked, which are intentionally NOT staged by `@coder`)
...
| 3 | Done | 2026-06-28 | 2026-06-28 | 4680012 | meeting guide before/during/after flowchart @L17 + back-link; header+marker intact. |
| 4 | Done | 2026-06-28 | 2026-06-28 | 05071c7 | decision guide D0–D14 kernel + R0–R3 routing @L17; 1 subgraph (<=2); no forbidden syntax; red "✗ Reopen" text cue; header+marker intact. |
| 5 | Done | 2026-06-28 | 2026-06-28 | f549ed3 | onboarding guide setup-flow @L17 + back-link; TOC intact (no new heading); header+marker intact. |
| 6 | Done | 2026-06-28 | 2026-06-28 | 4fa1e22 | back-links added to project-inception + change-lifecycle (other 3 done P3-5); all 5 guides link ados-processes.md; forward links added; doc-reconciliation = phase 7 anchor, no standalone guide. |
| 7 | Done | 2026-06-28 | 2026-06-28 | f438d21 | doc/00-index.md Start Here row added @L15 (<=50); link resolves; header+marker preserved. |
| 8 | Done | 2026-06-28 | 2026-06-28 | fcb7b7a | audited all 5 existing + new diagrams; palette already identical (#4CAF50/#2196F3/#FF9800/#F44336/#9C27B0); added concise Legend captions after the 4 project-inception fences (minor, no redesign); 0 forbidden syntax in-mermaid; canonical names consistent. |
| 9 | Done | 2026-06-28 | 2026-06-28 | 36adf71 | add-header-location.sh added the copyright header to ados-processes.md (1 updated/18 skipped); all 6 guides hdr=3 marker=1; doc/00-index marker=1; README header=3 no marker; only ados-processes.md body-untouched change. |
| 10 | Done | 2026-06-28 | 2026-06-28 | (pending) | doc-distribution exit 0 (76 docs, no drift); test-add-header-location 19/19; test-inception-doc-consistency exit 0; git diff --check exit 0; scope docs-only (0 .opencode/.ai/scripts/tools/doc/spec); no doc/spec reconciliation; no version bump; all 13 ACs PASSED. |
