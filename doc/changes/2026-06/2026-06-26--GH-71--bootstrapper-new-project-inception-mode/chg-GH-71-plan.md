...
| G | DONE | 2026-06-27 | 2026-06-27 | 87cdf07 (+RT3 follow-up) | Unification amendment (REM-9/REM-10) shipped. G.1 DONE: `@toolsmith` rewrote `bootstrapper.md` → unified `<inception_workflow>` (278 lines; 6-phase flow + `.ai/local/bootstrapper-context.yaml` eradicated; legacy front-half folded); updated `bootstrap.md` + README; regenerated `.ados-claude/`. G.2 DONE: feature spec rewritten; spec→v1.2 (AC16 superseded; AC18–23; DEC-8/DEC-9); TDR-0001 addendum. G.3 DONE: deleted `test-bootstrapper-prompt-structure.sh` (DEC-9). G.4 DONE: test-plan revised (structure layer superseded; TC-INCEP-017..022). G.5 DONE: `.ados-claude/` fresh; `test-doc-distribution.sh` + `test-build-claude-plugin.sh` green; no structure test. G.6 DONE: PR #82 body updated; GH-72 scope-absorption comment posted. **RT3 red-team (post-amendment): SHIP-WITH-FINDINGS → fixes applied: RT3-01 (P3 four-risk-on-architecture activity, prompt+guide), RT3-02 (allowlist `doc/decisions/**`), RT3-03 (AGENTS.md one-liner), RT3-04 (this row), RT3-06 (TDR historical marker); RT3-05/07 accepted/deferred.** |
