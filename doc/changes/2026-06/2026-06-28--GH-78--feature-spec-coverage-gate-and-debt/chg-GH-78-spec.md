...
- **`doc/spec/features` is created as an empty stub** by `install.sh` (`ADOS_LOCAL_DIRS`, install.sh line 121) — feature-spec **content is not redistributed**. Guide `status:` keys verified from frontmatter (not body templates): `doc/guides/change-lifecycle.md` and `doc/guides/definition-of-ready.md` are `status: Draft` (as are `definition-of-done.md` and `project-inception.md`); `doc/guides/decision-making.md` and `doc/guides/decision-records-management.md` carry **no `status:` key** in their frontmatter (both are `ados_distribution: redistributable`).
...
**F-3.2 model-configuration nuance (must be captured accurately):** `claude.model` in an agent's frontmatter is a **Claude-Code-targeted hint** consumed by `scripts/build-claude-plugin.sh` to assign the model in the generated plugin; the **OpenCode-effective** model assignment lives in the OpenCode config layer. **Critically, in this repo `.opencode/opencode.jsonc` does NOT contain a per-agent model assignment table** — its comment at line 25 explicitly delegates per-agent model assignments to user-global config ("rest (models etc) are customized in user-centric global configs so they don't have to be added below"). The spec therefore documents the **mechanism** (per `doc/guides/opencode-model-configuration.md`, which states that model assignments live exclusively in OpenCode config — NOT in agent definitions — and `AGENTS.md` "Multi-tool support") and must NOT imply `.opencode/opencode.jsonc` holds a literal per-agent model table. The two concerns (`claude.model` build-time hint vs OpenCode-effective runtime assignment) are independent. The spec must state this precisely and not conflate them.

**F-4 — Reconciliation.** Where a guide and a prompt disagree, the prompt wins (the DoR guide states this explicitly). Several guides are `status: Draft` (e.g., `change-lifecycle.md`, `definition-of-ready.md`; also `definition-of-done.md`, `project-inception.md`) — any divergence from a Draft guide must be flagged as a follow-up note rather than silently picking one. (`decision-making.md` carries no `status:` key and is not relied on as authoritative; prompts/`AGENTS.md` remain the source of truth.)
...
- **Model configuration is split across layers, and `.opencode/opencode.jsonc` holds no per-agent model table in this repo:** `claude.model` in `.opencode/agent/*.md` frontmatter is a Claude-Code-targeted build-time hint consumed by `scripts/build-claude-plugin.sh`; the OpenCode-effective per-agent model assignment lives in the OpenCode config layer — and `.opencode/opencode.jsonc` delegates per-agent assignments to user-global config (line 25), so it must not be cited as the literal location of a per-agent model table. Authoritative for this nuance: `doc/guides/opencode-model-configuration.md` + `AGENTS.md` ("Multi-tool support").
...
| AC-F3-3 | **Given** `feature-agents-and-commands.md`, **when** read, **then** it states the model-configuration nuance accurately (`claude.model` = Claude-Code hint consumed by the build script; OpenCode-effective assignment in the OpenCode config layer, with `.opencode/opencode.jsonc` delegating per-agent assignments to user-global config — no per-agent model table) and references `@toolsmith`. | F-3.2, NFR-1 |
