...
|----|---------|-------------|
| DM-1 | Consolidated review YAML schema | One YAML per review iteration (local `review-iter-<N>.yaml`; remote `review-draft.yaml`). **Identical schema both modes.** Top-level keys: `version: 1`; `iteration: <N>`; `mode: local \| remote`; `work_item_ref: <ref>`; `branch: <branch>`; `status: PASS \| FAIL`; `summary: <string>` (human-readable paragraph); `severity_breakdown: { critical, high, medium, low, info: <int> }`; `spec_compliance: PASS \| FAIL \| NA`; `plan_compliance: PASS \| FAIL \| NA`; `findings: [ { id: <F-n>, severity, confidence, category, location, message, suggestion, suppressed } ]`; `reviewed_at: <ISO8601>`; `next_step: <string>`. This single file supersedes the prior `findings-iter-<N>.json` (the structured `findings[]`) **and** `review-iter-<N>.md` (summary/audit/status). The `findings[]` array is the dedup source for re-review. **Old→new field mapping:** `file+line → location`; `title+description → message`; `suggestedFix → suggestion`; `confidence` and `suppressed` preserved from the prior JSON format (used for analysis weighting and remote dedup respectively). New field: `category`. |
...
| DEC-8 | Rescale severity enum from `critical/major/minor/nit` to `critical/high/medium/low/info` during the YAML consolidation. | The YAML schema redesign is a natural opportunity to align with the standard 5-tier severity convention. `info` replaces `nit` (clearer semantic). No programmatic consumer reads the old strings (RSK-1 confirms); historical artifacts untouched. Bundled with DM-1 consolidation rather than a separate change. | 2026-07-09 |
