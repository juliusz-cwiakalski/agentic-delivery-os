...
- **Install integration** (commits 05a469c, c103561, e559241): `install.sh --local` now installs delivery scripts/tools via `ADOS_DELIVERY_SCRIPTS`/`ADOS_DELIVERY_TOOLS` arrays; standalone `install-clean-merged-branches.sh` installer created; docs updated (standalone installer recommended, batch-delivery Installation section, AGENTS.md delivery-infrastructure registration note). All tests pass (11 + 21 + 25 + 15 = 72 tests; doc-distribution drift guard OK — 77 in-scope docs).
- **Red-team remediation batch** (commit 7209d23): LGTM security gate (opt-in/author-restricted/anchored), prompt-injection defense in review-comment handling, orphan cleanup via EXIT trap (`_cleanup_child`/`CURRENT_OPENCODE_PID`), uninstall symmetry.
- **Resume-sync + CI gates + mechanism tests + docs** (this commit): 
  - **Item 1 (CRITICAL)**: PM prompt now merges `origin/main` into the feature branch before resuming (catches breaking changes); fetches PR review comments on **every** resume (not only `CHANGES_REQUESTED`), with conflict/test-failure handling. New "Resume Sync" section + renumbered open-PR section. `${ticket_ref}/${branch}/${branch_hint}/${lgtm_signal}/${lgtm_rule_tail}` all expand correctly (verified).
  - **Item 2 (M-3)**: new CI jobs — `bash-tests` (runs all CI-safe `test-*.sh` via `find -print -exec`) and `shellcheck` (`severity: error`; 0 repo errors, ~126 pre-existing warnings deferred to a follow-up; paid-API e2e/perf suites excluded from `find`).
  - **Item 3 (m-6)**: deliver-ticket tests 28→35 — TC-DT-08h/i/j (merge-main, Resume Sync, comment-fetch prompt checks), TC-DT-CMP-01/02/03 (kill_process_tree, resolve_session title lookup, _cleanup_child EXIT-trap), TC-DT-INT-01 (full kill/restart cycle via fake opencode/gh stubs; gated behind `RUN_SLOW_TESTS=true`, validated end-to-end ~70s).
  - **Item 4 (m-2)**: activity-detection assumption callout added to `autonomous-batch-delivery.md`.
  - Verification: all 5 GH-124 suites green (11/21/35/15 + doc-distribution OK 77 docs); exact CI `find` command exits 0; CI YAML valid; shellcheck 0 errors on changed files.
