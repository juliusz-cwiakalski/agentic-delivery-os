...
- **Structured authoring (F-1):** Template with front-matter skeleton and the required body sections, driven by a **tiered-default applicability model** (rigor is the primary axis; type/archetype toggle only small enumerated add-ons — no full 2D matrix). The template tags each section by rigor applicability (`R1/R2/R3`, `R2/R3`, `R3-expanded`) and surfaces Decision Rights (DACI), Evidence/Assumptions/Unknowns, a single Decision section, eligibility-first alternatives, Rollback/Reversal, Communication Plan, and a Structured Retrospective. Worked R1/R2/R3 examples demonstrate the tiered-default rendering.
- **Classification block as routing home (F-9):** Records carry **no** top-level `decision_area` or `reversibility` — both live only inside the `classification:` block. `classification.reversibility` is the single canonical location for reversibility; `decision_area` was redundant with `decision_type` + `classification.domains`. No duplicate fields remain without a one-line justification.
- **Routing metadata via `classification` (F-10):** Records use the `classification:` block as the single canonical home for routing metadata — `domains`, `archetype`, `rigor`, `reversibility`, `stakes`, `urgency`, `uncertainty`. (GH-133 introduced this contract; GH-63 — machine-enforceable decision records — would build on it.)
...
- **Lifecycle management (F-2):** Status transitions from Proposed → Accepted → (Deprecated | Superseded). The optional `review_date` field drives an iterative review cycle — when it arrives, the decision is revisited and either reaffirmed, superseded, or deprecated.
...
The optional `review_date` field drives an iterative review cycle: when it arrives, the decision is revisited and either reaffirmed (status stays `Accepted`), superseded by a new record, or deprecated.
...
| `doc/guides/decision-records-management.md` | Record-artifact reference | Record standard: types, ADR/TDR tie-breaker + overlap guidance, naming, lifecycle, front matter (`classification` contract), governance (thin redirect to decision-making.md for process) |
| `doc/templates/decision-record-template.md` | Authoring template | Single source of truth for the record body: type-selection helper, front matter with `classification` as routing home, tiered-default applicability tags per section, Decision Rights, Evidence/Assumptions/Unknowns + Technical-Selection Evidence Pack, a single Decision section, eligibility-first alternatives, Rollback, Communication Plan, Structured Retrospective with iterative review, and worked R1/R2/R3 examples |
| `.opencode/agent/decision-advisor.md` | Decision advisor agent | Creates decision records; domain-neutral across all five types; targets `doc/decisions/`; delegates bounded evidence gathering to `@external-researcher` for selection decisions; applies the ADR/TDR tie-breaker and R1 default-local rule |
...
> **Front-matter contract (GH-133):** records carry **no** top-level `decision_area` or
> `reversibility` — both live only inside `classification:`.
...
10. Decision — the final decision plus the Constraint Compliance Attestation (R1/R2/R3)
...
| NFR-5 | Iterative review | The optional `review_date` field schedules when an Accepted decision is revisited (reaffirm / supersede / deprecate) | Accepted records may carry a `review_date` |
...
- **Record standard:** [doc/guides/decision-records-management.md](../../guides/decision-records-management.md) — record artifact standard (ADR/TDR tie-breaker, overlap guidance, `classification` front-matter contract)
