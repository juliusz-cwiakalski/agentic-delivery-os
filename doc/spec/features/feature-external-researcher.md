...
Decision-evidence mode           → @decision-advisor requests a bounded pack for archetype: selection
                                 → @external-researcher returns default top-3 candidates (expand when warranted) × ≤10 signals
...
- **Risk:** Paid-service quota misuse; mitigated by preferring single-source queries, the evidence-pack guideline (default top-3 × ~10), and avoiding unnecessary parallel calls.
