...
owners: ["engineering"]
service: delivery-os
summary: "The .opencode/ agent and command system is the product: a single source of truth for role definitions and composed workflows, with bounded knowledge handoffs, a precise two-layer model-configuration mechanism, and generated multi-tool parity."
links:
...
- **Project-knowledge interfaces (F-7):** `@knowledge` is the shared facade for evidence-backed project questions, bounded knowledge-health review, Contributor Orientation, and durable Knowledge Gap stewardship. Expert consumers query the agent directly. `/knowledge-review` and `/contributor-orientation` are non-interactive command interfaces that explicitly delegate to `knowledge`; they do not duplicate its semantic contract or silently fall back to a default assistant. Canonical inventory and usage detail remain in [AGENTS.md](../../../AGENTS.md), [.opencode/README.md](../../../.opencode/README.md), and the [Project Knowledge Management guide](../../guides/project-knowledge-management.md).
- **Bounded knowledge handoffs (F-8):** A role uses known canonical context first and requests knowledge only for material unresolved project facts. The request carries an owning role, bounded question/task intent, knowledge class, finite source scope, checked evidence, unresolved uncertainty, requested outcome, consumer/destinations, capture mode, `knowledge_depth`, and `visited_roles`. A fresh handoff reaches `@knowledge` once at depth 1; knowledge acts as a leaf, returns bounded evidence and recommended routing, and never calls itself or another owning role. The caller owns continuation. Missing delegation tooling produces a bounded parent-broker packet rather than role impersonation or recursive tool acquisition.
...
| `.opencode/agent/knowledge.md` | Knowledge facade | Query, review, orientation, gap capture, and verification semantics |
| `.opencode/command/{knowledge-review,contributor-orientation}.md` | Knowledge workflows | Thin composed entry points that explicitly target `knowledge` |
| `.ai/agent/knowledge-instructions.md` | Optional project policy | Source authority/access/disclosure, ownership/escalation, and capture configuration; not an answer source |
...
| NFR-4 | Handoff safety | Knowledge delegation is bounded, one-depth, non-recursive, and caller-owned | No self-delegation or role bounce |
| NFR-5 | Interface parity | Canonical and generated knowledge agent/command interfaces stay discoverable and equivalent | Inventory and generated-plugin checks pass |
...
| Contract | Knowledge inventory and interfaces | `scripts/.tests/test-knowledge-contracts.sh` verifies canonical/generated presence, command targeting, inventories, and shipped support artifacts |
...
- **Project Knowledge Management:** [feature-project-knowledge-management.md](feature-project-knowledge-management.md) — authority, uncertainty, gap, review, orientation, and handoff contracts.
