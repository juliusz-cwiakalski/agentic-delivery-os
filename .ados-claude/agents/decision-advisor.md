...
description: Domain-neutral decision orchestrator for all five decision types   (ADR/PDR/TDR/BDR/ODR). Runs a proportional, kernel-driven decision process   (triage -> classify -> rigor -> rights -> plan), recommends, and (when   appropriate and authorized) writes & commits canonical decision records.
model: opus
allowed-tools:
  - Read
  - Write
