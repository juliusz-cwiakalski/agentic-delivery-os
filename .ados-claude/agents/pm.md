...
# Source of truth: .opencode/agent/pm.md
...
- **Delivery mode (`delivery_mode`):** At intake (clarify_scope, step 3), record `delivery_mode` in `chg-<workItemRef>-pm-notes.yaml`: `interactive | autonomous`. An **absent** field is treated as `interactive` (the default — backward-compatible, no migration). Manual `@pm` invocations set it explicitly (or leave it absent ⇒ `interactive`); the autonomous session entry point (`scripts/opencode-session.sh`) instructs `@pm` to set `autonomous`. The field is an optional, backward-compatible per-change signal; it does **not** gate spec-coverage resolution (see next bullet + `.opencode/agent/doc-syncer.md`) — it is reserved for future mode-aware features.
- **Feature spec coverage awareness:** For each **feature area** the change modifies (a coherent, nameable capability a contributor/reviewer would expect a spec for — i.e., something that warrants a `doc/spec/features/feature-<slug>.md`), note whether a corresponding spec exists in `doc/spec/features/`. Record any known coverage gap in `chg-<workItemRef>-pm-notes.yaml` so it is visible before delivery. **Phase 7 ALWAYS resolves any detected spec-coverage gap in-change** (authoritative detail in `.opencode/agent/doc-syncer.md`): `@doc-syncer` authors the missing first `feature-<slug>.md` in **every** mode (first-spec-only; an existing spec is reconciled); no human decision, no follow-up ticket. The human alone decides on OTHER follow-up tickets (but NOT spec coverage — that is unconditional now). No tracker ticket is created in any mode.
