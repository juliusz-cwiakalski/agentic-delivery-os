...
_hook_file_metadata() { local -r file="$1"; local hook_stat_output; if hook_stat_output="$(_hook_stat -c '%a %u' "${file}" 2>/dev/null)"; then printf '%s' "${hook_stat_output}"; else _hook_stat -f '%Lp %u' "${file}" 2>/dev/null; fi; }
